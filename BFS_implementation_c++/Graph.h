...
class Graph {
    ...
    //TO DO: Add isntance variables.
    //Think about what data structures are needed for a graph

    //Idea: adjacency list(which is in fact a double dictionary behind-the-scenes) to store edges and weights
    private:
        // member variables (aka instance variables)

        //potential problem: might need to use pointer(*) instead.
        unordered_map<string,unordered_map<string,double>> adjacencylist; //stores all neighbors of each node with corresponding weights.
    ...
    public:
        ...
        /**
         * Return a map of all nodes and their visited status (0 means not, 1 means yes) in this graph, in any order.
         * @return A `vector` containing the labels of all nodes in this graph, in any order.
         */
